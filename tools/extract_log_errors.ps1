<#
.SYNOPSIS
    Extracts TaxMod-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the TaxMod mod, categorizing them by severity and type.
...
Write-Header "TaxMod Log Analyzer"
...
# Patterns to match
$patterns = @{
    # TaxMod specific patterns
    TaxModLine = 'Tax Mod|FS25_TaxMod'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is TaxMod related
    if ($line -match $patterns.TaxModLine) {
        $stats.TaxModLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'TaxMod') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report += "TaxMod-Related Lines: $($stats.TaxModLines)"
